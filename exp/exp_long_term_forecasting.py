...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        # avg_inference_time_per_iter = total_inference_time / total_batches
        # logger.info(f'Average inference time per iteration: {avg_inference_time_per_iter:.2f} ms')
    ...
    def train(self, setting):
        ...
        # print("===============> train_data: ", len(vali_data))
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                # encoder - decoder
                # Start timing training iteration
                start_time = time.time()
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        if self.args.output_attention:
                            # print("============> batch_x:, x_mark ", batch_x.shape, batch_x_mark.shape, dec_inp.shape, batch_y_mark.shape)
                            outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)[0]
                        else:
                            # print("============> batch_x:, x_mark ", batch_x.shape, batch_x_mark.shape, dec_inp.shape,
                            #       batch_y_mark.shape)
                            outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)
    ...
    def test(self, setting, test=0):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                # print("=========== testing: ", outputs.shape, batch_y.shape)
        ...
        # result save
        name = self.args.data_path.split(".")[0]
        folder_path = f"./FAT_results/{name}_outcome/{setting}/"
